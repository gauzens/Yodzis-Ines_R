...
public:
  ...
  NumericVector K2 = NumericVector(nb_b);
  ...
  double F_rate(int prey, int pred, NumericVector bioms){
    ...
    int i;
    // double res = 0;
    //     return ((w(pred)*b(prey,pred)*pow(bioms(prey + nb_n),1+q)) / 
    // ((1 + c*bioms(pred + nb_n) + w(pred)*h(prey,pred)*sum(b(_,pred)*pow(bioms[Range(nb_n, nb_s + nb_n-1)],1+q)))*BM(pred)));

    for (i=0; i<nb_s; i++){
      tot += b(i,pred) * pow(bioms[i+nb_n], 1+q);
    }
    return ((w(pred)*b(prey,pred)*pow(bioms(prey + nb_n),1+q)) / 
    ((1 + c*bioms(pred + nb_n) + w(pred)*h(prey,pred)*tot)*BM(pred)));
  }
  
  // NumericVector ODE(double t, NumericVector bioms){ // for sundials
  NumericVector ODE(NumericVector bioms, double t){ //for deSolve or odeintr
    ...
    uptake =  r[plants]*bioms[plants_bioms]; 
    ...
    dB(1) = D * (S(1) - bioms(1)) - v2*sum(uptake);
